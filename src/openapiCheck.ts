...
function collectOperationIdsFromOpenApi(doc: OpenApiDoc): {
  ...
  for (const [p, pathItem] of Object.entries(paths) as [
    string,
    OpenApiPathItem,
  ][]) {
    for (const [methodRaw, op] of Object.entries(pathItem) as [
      string,
      OpenApiOperation,
    ][]) {
...
export async function openapiCheck(
  opts: OpenapiCheckOptions,
  ...
  const { opIds, duplicates, missing } = collectOperationIdsFromOpenApi(
    openapiParsed.data,
  );
