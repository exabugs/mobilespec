...
type L3 = {
  screen: { id: string; context?: string; layout: Record<string, unknown> };
};
...
export async function generateI18n(options: Options): Promise<void> {
  ...
  const cfg = fs.existsSync(cfgPath)
    ? (yaml.load(fs.readFileSync(cfgPath, "utf8")) as Record<string, unknown>)
    : {};
  const paths =
  ...
  const L3_DIR = path.join(
    options.specsDir,
    typeof paths.l3 === "string" ? paths.l3 : "L3.ui",
  );
